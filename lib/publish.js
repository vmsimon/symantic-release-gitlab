...
export default async (pluginConfig, context) => {
  ...
  const { gitlabJobToken, gitlabToken, gitlabUrl, gitlabApiUrl, assets, milestones, proxy } = resolveConfig(
    pluginConfig,
    context
  );
  const assetsList = [];
  ...
  const tokenHeader = isNil(gitlabJobToken) ? "PRIVATE-TOKEN" : "JOB-TOKEN";
  const tokenValue = isNil(gitlabJobToken) ? gitlabToken : gitlabJobToken;
  const apiOptions = {
    headers: {
